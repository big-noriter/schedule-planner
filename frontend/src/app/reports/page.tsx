...
export default function ReportsPage() {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      ...
      try {
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}/api/analytics/reports`, {
          method: 'POST',
          headers: {
  ...
  const handleDownload = async (pdfUrl: string) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}${pdfUrl}`);
