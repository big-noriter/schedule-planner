...
export default function DepartmentAnalytics() {
  ...
  useEffect(() => {
          fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}/api/analytics/departmentTasks`)
      .then(res => res.json())
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}/api/analytics/generateReport`, {
        method: 'POST',
        headers: {
